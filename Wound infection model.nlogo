...
to setup
  ...
  if Diabetes [set Immune-Cell-Infiltration 2] ;NEW LINE
  if Diabetes [set Macrophage_Response "Low"]
  ...
  ask neutrophils
    [
      ...
      ifelse Diabetes [set age 15] [set age 1] ;CHANGED
      ; set age 1
    ]
...
to Run-Simulation
  ...
  if any? bacteria [
    ...
    ;if Diabetes [create-macrophages ceiling (2 * Immune-Cell-Infiltration) [setxy random-xcor random-ycor setxy pycor pxcor set shape "macrophage" set age 1]] ;CHANGED
  ]
...
@#$#@#$#@
<experiments>
  <experiment name="BME_6311 Test 2" repetitions="3" runMetricsEveryStep="true">
    <setup>setup
infect</setup>
    <go>run-simulation</go>
    <timeLimit steps="168"/>
    <exitCondition>count bacteria = 0</exitCondition>
    ...
    <enumeratedValueSet variable="Diabetes">
      <value value="false"/>
      <value value="true"/>
    </enumeratedValueSet>
  </experiment>
